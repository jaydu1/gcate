...
for(cell_type in cell_types){
    ...
    X <- cbind(rep(1, length(log_libsize)), log_libsize, X)
    X <- data.frame(X)
    ...
    for(calibrate in c(TRUE,FALSE)){
        cate.results <- cate(~ X8 | . - X8, X, log2(Y+1), r = factor.num$r, calibrate=calibrate)
        ...
        if(calibrate){
            filename <- sprintf('%scate_full_%s.csv', path_result, cell_type)
        }else{
            filename <- sprintf('%scate_full_raw_%s.csv', path_result, cell_type)
        }
